...
# Currently Formula does not support moving the NiFi executables.
...
nifi-config-file-file-managed:
  file.managed:
  {% if pillar.nifi.bootstrap['conf.dir'] is defined %}
...
nifi-bootstrapnotifications-file-file-managed:
  file.managed:
  {% if pillar.nifi.bootstrap['notification.services.file'] is defined %}
...
nifi-authorizers-file-file-managed:
  file.managed:
  {% if pillar.nifi.nifi['authorizer.configuration.file'] is defined %}
...
nifi-loginprovider-file-file-managed:
  file.managed:
  {% if pillar.nifi.nifi['login.identity.provider.configuration.file'] is defined %}
...
nifi-statemanagement-file-file-managed:
  file.managed:
  {% if pillar.nifi.nifi['state.management.configuration.file'] is defined %}
...
nifi-zookeeper-file-file-managed:
  file.managed:
  {% if pillar.nifi.nifi['state.management.embedded.zookeeper.properties'] is defined %}
